local Beastmaster = {
    entry = 101032, -- Beastmaster entry.
    maxObj = 10, -- 13 = Max amt. of menu objects.
}
...
function Beastmaster.OnHello(event, player, unit)
    ...
    if(player:GetClass() == 3) then
        ...
        if(player:HasSpell(1515)) then
            ...
            player:SendBroadcastMessage("[Squire Conanus]: Vous n'avez pas cette capacitée.")
        end
    else
        player:SendBroadcastMessage("[Squire Conanus]: Seulement les chasseurs peuvent avoir un familier !")
    end
end
...
function Beastmaster.OnSelect(event, player, unit, sender, intid, code)
    ...
    elseif(intid == 1) then
        ...
        local pet = PerformIngameSpawn(1, sender, unit:GetMapId(), unit:GetInstanceId(), unit:GetX(), unit:GetY(), unit:GetZ(), unit:GetO(), false, 2000)
        pet:SetFaction(35)
...
function Beastmaster.LoadCache()
    ...
    if(Query) then
        ...
        print("[Squire Conanus]: Cache initialized. Loaded "..Query:GetRowCount().." tameable beasts.")
    else
        print("[Squire Conanus]: Cache initialized. No results found.")
    end
end
