...
local function OnGossipHello(event, player, object, unit)
	...
	local pLevel = player:GetLevel();
		if(pLevel == 1)then
			player:GossipMenuAddItem(0, "Suivre le petit guide de départ", 1, 10)
			player:GossipMenuAddItem(0, "Commencer à jouer", 1, 11)
...
local function OnGossipSelect(event, player, object, sender, intid, code)
	...
	if (intid == 1) then
		local pName = player:GetName();
		player:SendBroadcastMessage(pName.." vous commencez niveau 1.")
		player:SetLevel(1);
		player:Teleport( 1, -6541.454102, 2853.834473, 2.278454, 1.501072 )
		player:GossipComplete();
	elseif (intid == 2)then
		local pName = player:GetName();
		player:SendBroadcastMessage(pName.." vous commencez niveau 80.");
		player:SetLevel(80);
	...
	elseif(intid == 11)then
		player:GossipMenuAddItem(0, "|TInterface\\icons\\UI_Promotion_CharacterBoost:40:40:-22:0|t Commencer niveau 1", 1, 1, false, "Etes vous sûr ?")
        player:GossipMenuAddItem(0, "|TInterface\\icons\\UI_Promotion_CharacterBoost:40:40:-22:0|t Commencer niveau 80", 1, 2, false, "Etes vous sûr ? Vous n'aurez pas la récompense de la zone xp.")
		player:GossipSetText("Très bien ! Que souhaites-tu faire ?")
		player:GossipSendMenu(0x7FFFFFFF, object, MenuId)
	...
	elseif(intid == 15)then
		player:GossipMenuAddItem(0, "Très bien, je pense pouvoir commencer l'aventure !", 1, 11)
