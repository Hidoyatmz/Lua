...
local NpcpveLevel = 101074;

local function pveLevel(event, player, object)
	local pName = player:GetName();
	local pGuid = player:GetGUIDLow();
	local pvelevel = GetPlayerPvELevel(pGuid);
	local pvexp = GetPlayerPvEXp(pGuid);
	local pvenextxp = GetPlayerNextXpPvE(pGuid);
	local pvenextlevel = pvelevel+1;
	local GetRequiresQuery = WorldDBQuery("SELECT * FROM pve_level_requirements WHERE level = "..pvenextlevel);
	if(GetRequiresQuery)then
		...
		player:GossipMenuAddItem(0, "Experience requise: "..pvexp.."/"..row.xp_requires, 1, 0);
		...
		player:GossipMenuAddItem(0, "|TInterface\\icons\\UI_Promotion_CharacterBoost:40:40:-22:0|t Passer au niveau : "..pvenextlevel, 1, 1);
		player:GossipSetText("Bonjour "..pName.."\n\nNiveau suivant: "..pvenextlevel);
		player:GossipSendMenu(0x7FFFFFFF, object, MenuId)
	else
		player:SendBroadcastMessage("Vous avez atteint le niveau maximum en pve...")
	end
end

local function pvelevelUp(event, player, object, sender, intid, code, menu_id)
	if(intid == 1)then
		local pName = player:GetName();
		local pGuid = player:GetGUIDLow();
		local pvelevel = GetPlayerPvELevel(pGuid);
		...
		if(pvexp > row.xp_requires)then
				...
				if(player:HasItem(row.item2, row.item2_count))then
					if(row.item3 ~= 0)then
						if(player:HasItem(row.item3, row.item3_count))then
							...
							UpdatePlayerPvELevel(pGuid, 1);
							player:SendBroadcastMessage("Félicitation vous avez atteint le niveau "..pvenextlevel.." en pve.")
			...
			player:SendBroadcastMessage("Vous n'avez pas assez d'xp pve pour passer au prochain niveau...")
		end
		player:GossipComplete();
	elseif(intid == 0)then
		pveLevel(event, player, object)
	end
end

RegisterCreatureGossipEvent(NpcpveLevel, 1, pveLevel)
RegisterCreatureGossipEvent(NpcpveLevel, 2, pvelevelUp)